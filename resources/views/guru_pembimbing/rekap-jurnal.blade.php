...
@section('header_title', 'JURNAL KEGIATAN SISWA')
...
<div class="space-y-6">
    ...
    <div class="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-white/40 shadow-sm flex items-center justify-between flex-wrap gap-4">
        <form action="{{ route('guru_pembimbing.rekap-jurnal') }}" method="GET" class="flex items-center gap-3">
            <div class="relative">
            ...
            <a href="{{ route('guru_pembimbing.rekap-jurnal') }}" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold rounded-xl transition">
                Reset
            </a>
        </form>
    </div>
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                <button type="button" 
                                        ...
                                        data-action="{{ route('guru_pembimbing.rekap-jurnal.verify', $jrn->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-gray-500">
                                <p class="text-lg font-semibold text-gray-600">Belum ada data jurnal harian.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden transform transition-all scale-95 opacity-0 duration-300 z-10" id="detailJurnalContent">
        <form id="verifyForm" method="POST" class="m-0">
            ...
            <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <!-- Catatan Pembimbing (Textarea) -->
                <div class="py-2">
                    <label for="modalCatatan" class="text-xs font-bold text-emerald-700 block mb-1.5 flex items-center gap-1">
                        ✍️ Berikan Catatan / Komentar
                    </label>
                    <textarea name="catatan_pembimbing" id="modalCatatan" rows="3" 
                              placeholder="Tulis saran, revisi, atau catatan bimbingan Anda di sini..."
                              class="w-full text-sm border border-gray-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-emerald-500 outline-none transition duration-200"></textarea>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                <button type="button" onclick="closeJurnalDetail()" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 font-semibold rounded-xl text-sm transition">
                    Tutup
                </button>
                <button type="submit" class="px-5 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-bold rounded-xl text-sm shadow-md shadow-emerald-500/10 transition">
                    Simpan Catatan
                </button>
            </div>
        </form>
...
function showJurnalDetail(btn) {
    ...
    document.getElementById('verifyForm').action = data.action;
    ...
    // Set textarea value
    document.getElementById('modalCatatan').value = data.catatan;
