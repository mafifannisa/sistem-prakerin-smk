...
<head>
    ...
    <title>@yield('title', 'Dashboard Guru Pembimbing') - Sistem Prakerin SMK N 3 Tuban</title>
    ...
    <div class="flex min-h-screen relative p-0 lg:p-4">
        ...
        <aside id="sidebar" class="w-64 bg-white/85 backdrop-blur-md border-r border-gray-150 lg:border lg:border-white/40 shadow-xl lg:shadow-[0_10px_30px_rgba(0,0,0,0.04)] fixed inset-y-0 left-0 lg:left-4 lg:top-4 lg:bottom-4 z-50 h-full lg:h-[calc(100vh-2rem)] overflow-y-auto transform -translate-x-full lg:translate-x-0 transition-all duration-300 ease-in-out flex flex-col rounded-none lg:rounded-2xl">
            ...
            <nav class="p-4 space-y-1.5 flex-1 overflow-y-auto">
                ...
                <a href="{{ route('guru_pembimbing.dashboard') }}" 
                   class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-250 {{ request()->routeIs('guru_pembimbing.dashboard') ? 'sidebar-active' : 'text-gray-655 hover:bg-orange-50/50' }}">
                ...
                <a href="{{ route('guru_pembimbing.data-siswa') }}" 
                   class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-250 {{ request()->routeIs('guru_pembimbing.data-siswa') ? 'sidebar-active' : 'text-gray-655 hover:bg-orange-50/50' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="font-semibold text-sm">Siswa Bimbingan</span>
                </a>

                <a href="{{ route('guru_pembimbing.rekap-absen') }}" 
                   class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-250 {{ request()->routeIs('guru_pembimbing.rekap-absen') ? 'sidebar-active' : 'text-gray-655 hover:bg-orange-50/50' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('guru_pembimbing.rekap-jurnal') }}" 
                   class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-250 {{ request()->routeIs('guru_pembimbing.rekap-jurnal') ? 'sidebar-active' : 'text-gray-655 hover:bg-orange-50/50' }}">
                    ...
                    <span class="font-semibold text-sm">Rekap Jurnal</span>
                </a>

                <a href="{{ route('guru_pembimbing.rekap-laporan') }}" 
                   class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-250 {{ request()->routeIs('guru_pembimbing.rekap-laporan') ? 'sidebar-active' : 'text-gray-655 hover:bg-orange-50/50' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('guru_pembimbing.laporan-masalah') }}" 
                   class="nav-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-250 {{ request()->routeIs('guru_pembimbing.laporan-masalah') ? 'sidebar-active' : 'text-gray-655 hover:bg-orange-50/50' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                    </svg>
                    <span class="font-semibold text-sm">Laporkan Masalah</span>
                </a>
            </nav>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex flex-col">
                            <div class="flex items-center gap-1.5 text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-0.5">
                                ...
                                <span class="text-orange-500">@yield('header_breadcrumb', 'Guru Pembimbing')</span>
                            </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="relative">
                                <button onclick="toggleUserDropdown()" class="w-10 h-10 bg-gradient-to-tr from-orange-500 to-amber-400 rounded-full flex items-center justify-center text-white font-bold shrink-0 shadow-sm border border-white hover:scale-105 active:scale-95 transition-all duration-200">
                                    {{ substr(Auth::user()->nama_lengkap ?? 'G', 0, 2) }}
                                </button>
                                    ...
                                    <div class="p-5 border-b border-gray-100 bg-gradient-to-br from-orange-50/30 to-orange-100/10">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="text-[10px] text-gray-400 font-semibold uppercase">Guru Pembimbing</p>
                                            </div>
                                        </div>
                                    </div>
